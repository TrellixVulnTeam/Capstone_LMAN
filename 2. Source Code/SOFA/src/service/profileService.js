...
export const getOtherProfile = (id) => {
    return new Promise((resolve, reject) => {
        ...
        var uri = Const.domain + 'api/profile/otherprofile?id=' + id;
        Request.Get(uri, header)
            .then(response => {
                resolve(response);
            })
...
export const searchUserByName = (keyword, page) => {
    return new Promise((resolve, reject) => {
        var header = {
            "User-Agent": 'Mozilla/5.0 (Linux; Android 10) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/87.0.4280.141 Mobile Safari/537.36',
            "Accept": 'application/json',
        };
        var uri = Const.domain + 'api/profile/SearchUserByName?keyword=' + keyword + '&page=' + page + '&rowsOfPage=' + Const.NEWSFEED_ROWS_OF_PAGE;
        Request.Get(uri, header)
            .then(response => {
                resolve(response);
            })
